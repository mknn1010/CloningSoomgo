...
<%	
	int categoryIdx = 1;
	try{ 
		categoryIdx = (Integer) request.getAttribute("categoryIdx"); 
	...
	try {
		ecDao = (EstimateCategoryDao) request.getAttribute("ecDao");
		if(ecDao == null){
			System.out.println("ecDao null");
		}
		listCategory = (ArrayList<EstimateCategoryDto>) request.getAttribute("listCategory");
		if(listCategory == null){
			System.out.println("listCategory null");
		}
		mDao = (EstimateMidCategoryDao) request.getAttribute("mDao");
		if(mDao == null){
			System.out.println("mDao null");
		}
		listMidCategory = (ArrayList<EstimateMidCategoryDto>) request.getAttribute("listMidCategory");
		if(listMidCategory == null){
			System.out.println("listMidCategory null");
		}
		esDao = (EstimateServiceDao) request.getAttribute("esDao");
		if(esDao == null){
			System.out.println("esDao null");
		}
	} catch(Exception e) {
		System.out.println("RequestMain : catch 패스");
		e.printStackTrace();
	}
			...
			<div class=" header">
				...
				<div class=" category_container">
					<ul class=" category_list">
						<%
							for(EstimateCategoryDto ecDto : listCategory) {
						%> 
						<li class="category_item fl">
							<a class="router_link" href="EstimateController?command=category_list&categoryIdx=<%=ecDto.getCategoryIdx()%>">
								<div class="category_icon">
									<div class="category_box">
										<div class="category_img_wrapper">
											<img src="<%=ecDto.getThumbnail()%>"/>
										</div>
										<span class="category_name"><%=ecDto.getTitle()%></span>
									</div>
								</div>
							</a>
						</li>
				...
				<div class=" service_list_wrapper fl">
					<ul class=" service_list_container fl">
						...
						<li class=" service_wrapper fl" id="mov">
							...
							<%
								ArrayList<EstimateServiceDto> serByMidCateIdx = esDao.getServiceByMidCategoryIdx(mDto2.getMidCatergoryIdx());
							%>
